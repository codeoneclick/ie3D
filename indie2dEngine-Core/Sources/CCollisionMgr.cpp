...
void CCollisionMgr::_OnInputTapRecognizerPressed(const glm::ivec2& _point)
{
    ...
    {
        std::vector<std::shared_ptr<IGameObject> > targets = handler->_Get_Commands()._ExecuteGetTargetsCommand();
        for(auto target : targets)
        {
            std::shared_ptr<CMesh> mesh = target->Get_Mesh();
            assert(mesh != nullptr);
            glm::vec3 point;
            if(CCollisionMgr::_CollisionPoint(mesh->Get_VertexBuffer(), mesh->Get_IndexBuffer(), target->Get_WorldMatrix(), origin, direction, &point))
            {
                handler->_Get_Commands()._ExecuteCollisionCommand(point, target);
            }
        }
    }
}
...
void CCollisionMgr::Unproject(const glm::ivec2& _point, const glm::mat4x4& _viewMatrix, const glm::mat4x4& _projectionMatrix, glm::ivec4 _viewport,glm::vec3* _origin, glm::vec3* _direction)
{
    ...
    (*_origin) = origin;
    std::cout<<"[CCollisionMgr::Unproject] Origin : ( "<<origin.x<<", "<<origin.y<<", "<<origin.z<<" )"<<std::endl;
    std::cout<<"[CCollisionMgr::Unproject] Direction : ( "<<direction.x<<", "<<direction.y<<", "<<direction.z<<" )"<<std::endl;
}
