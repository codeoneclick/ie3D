//
//  CConfigurationParticleEmitterLoader.cpp
//  indie2dEngine
//
...
CConfigurationParticleEmitterLoader::CConfigurationParticleEmitterLoader(void)
{
...
void CConfigurationParticleEmitterLoader::Load(const std::string& _filename, std::shared_ptr<ITemplateLoadingHandler> _handler)
{
    ...
    std::function<void(std::string, std::shared_ptr<ITemplateLoadingHandler>)> function = [](std::string _filename, std::shared_ptr<ITemplateLoadingHandler> _handler)
    {
        ...
        std::shared_ptr<CConfigurationParticleEmitter> particleEmitterTemplate = std::static_pointer_cast<CConfigurationParticleEmitter>(operation->Serialize(_filename));
        assert(particleEmitterTemplate != nullptr);
        assert(_handler != nullptr);
        
#if defined(__USE_GCDPP__)
        std::function<void(std::shared_ptr<ITemplateLoadingHandler>, std::shared_ptr<CConfigurationParticleEmitter>)> function = []( std::shared_ptr<ITemplateLoadingHandler> _handler, std::shared_ptr<CConfigurationParticleEmitter> _template)
        {
