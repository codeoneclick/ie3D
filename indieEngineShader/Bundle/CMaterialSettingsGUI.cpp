...
void CMaterialSettingsGUI::Set_Transition(const std::shared_ptr<CIESAMainTransition> _ieasTransition)
{
    ...
    std::function<void(const std::shared_ptr<IResource>&)> function = [handler, this](const std::shared_ptr<IResource>& _resource)
    {
        ...
        ui->cb_blending_function_source->setCurrentText(g_glenumToString[material->Get_BlendFunctionSource()].c_str());
        ui->cb_blending_function_destination->setCurrentText(g_glenumToString[material->Get_BlendFunctionDestination()].c_str());
