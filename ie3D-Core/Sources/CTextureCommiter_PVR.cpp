...
void CTextureCommiter_PVR::commit(void)
{
	...
	for (ui32 mip = 0; mip < texture->getNumMips() && width > 0 && height > 0; ++mip)
	{
        ...
        ieCompressedTexImage2D(GL_TEXTURE_2D, mip, texture->getFormat(), width, height, 0, size, data) :
        ieTexImage2D(GL_TEXTURE_2D, mip, texture->getFormat(), width, height, 0, texture->getFormat(), GL_UNSIGNED_BYTE, data);
		data += size;
		width >>= 1; height >>= 1;
	}
