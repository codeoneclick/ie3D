...
CSharedTexture CResourceLoader::startTextureLoadingOperation(const std::string& filename)
{
    ...
    {
        resource = std::static_pointer_cast<CTexture>(m_resources.find(guid)->second);
    }
    else
    {
...
CSharedShader CResourceLoader::startShaderLoadingOperation(const std::string& vsFilename, const std::string& fsFilename)
{
    ...
    if(m_resources.find(guid) != m_resources.end())
    {
        resource = std::static_pointer_cast<CShader>(m_resources.find(guid)->second);
    }
    else
    {
...
CSharedMesh CResourceLoader::startMeshLoadingOperation(const std::string& filename)
{
    ...
    if(m_resources.find(guid) != m_resources.end())
    {
        resource = std::static_pointer_cast<CMesh>(m_resources.find(guid)->second);
    }
    else
    {
...
CSharedAnimationSequence CResourceLoader::startAnimationSequenceLoadingOperation(const std::string& filename)
{
    ...
    if(m_resources.find(guid) != m_resources.end())
    {
        resource = std::static_pointer_cast<CAnimationSequence>(m_resources.find(guid)->second);
    }
    else
    {
...
void CResourceLoader::removeLoadingDependecies(ISharedResourceLoadingHandler handler)
{
    for(std::pair<std::string, ISharedResource> it : m_resources)
    {
        it.second->removeLoadingHandler(handler);
    }
}
