...
void CKOTHInGameScene::Load(void)
{
    ...
    std::shared_ptr<CModel> model = m_root->CreateModel("model.character.xml");
    ...
    for(i32 i = 0; i < 14; ++i)
    {
        for(i32 j = 0; j < 14; ++j)
        {
            ...
            {
                std::shared_ptr<CModel> model = m_root->CreateModel("model.cube.xml");
    ...
    std::shared_ptr<CParticleEmitter> particleEmitter = m_root->CreateParticleEmitter("particle.xml");
...
void CKOTHInGameScene::OnUpdate(f32 _deltatime)
{
    ...
    m_models[0]->Set_Animation("character.MDL_anim");
    
    for(ui32 i = 1; i < m_models.size(); ++i)
    {
        if(i % 2 == 0)
        {
            m_models[i]->Set_Animation("cube.MDL_anim");
        }
    }
    angle += 0.33f;
